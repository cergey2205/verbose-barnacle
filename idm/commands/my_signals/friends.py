...
def add_to_fr(event: MySignalEvent) -> str:
    if event.reply_message == None:
        edit_message(event.api, event.chat.peer_id, event.msg['id'], message="❗ Ошибочка при выполнении, необходимо пересланное сообщение")
        return "ok"
    ...
    try:
        ...
        if e.error_code == 174:
            edit_message(event.api, event.chat.peer_id, event.msg['id'],
            message="❗ Невозможно добавить в друзья самого себя (Норм? Ну это так)")
